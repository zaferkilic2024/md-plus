...
export class Suggestion {
   ...
   *   (KR-49).
   */
  async run(view, job, { source: whole = null, options = undefined } = {}) {
    // `view` is null on a PDF: there is no editor there and there never will be
    // (KR-68). Everything the view is for — the selection, the accept, watching
    // the paragraph for edits — is meaningless on a page that cannot be written
    // to, so each of them asks first. A PDF hands its text in as `source`.
    const range = view ? view.state.selection.main : null;
    const source = whole ?? (range ? view.state.sliceDoc(range.from, range.to) : "");
    if (!source.trim()) return;

    this.close();
    this.view = view;
    this.range = whole || !range ? null : { from: range.from, to: range.to };
    this.job = job;
    this.source = source;
    ...
    this.docAtStart = view ? view.state.doc.toString() : null;
    ...
    const sonuc = await suggest(job, source, { signal: this.controller.signal, options });
    this.controller = null;
    clearInterval(this.timer);

    if (sonuc.iptal) return this.close();

    if (view && view.state.doc.toString() !== this.docAtStart) {
  ...
  showReport({ rapor, sure, kullanim, model, via, uzun }) {
    ...
    // A list job answers with alternatives, and an alternative is a thing you
    // take one of — so each line carries its own copy. Everything else is one
    // text and gets the single copy at the foot.
    //
    // The prompt asks for exactly three lines; if the model sends two or five,
    // the card shows what came. It does not pad, cut or renumber — inventing a
    // third title would be the app writing, and the app does not write.
    const satirlar = jobLines(this.job) ? candidateLines(rapor) : [];

    if (satirlar.length) {
      govde.classList.add("report-list");
      for (const satir of satirlar) {
        const sira = document.createElement("div");
        sira.className = "report-line";

        const metin = document.createElement("span");
        metin.className = "report-line-text";
        metin.textContent = satir;

        sira.append(metin, copyButton(satir));
        govde.append(sira);
      }
    } else {
