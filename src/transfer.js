...
// needs two documents on screen at once. So the source is read-only, and a
// selection in it opens nothing: no palette, no button, no offer (UC-12-K7).
...
export class Transfer {
  ...
  chooseTarget() {
    ...
    const others = this.tabs().filter((tab) => tab !== this.source); // UC-12/A3: never itself
  ...
  async bind() {
    ...
    setReadOnly(this.source.view, true); // UC-12-K4
    ...
    // the face that sends (KR-55, KR-56).
  ...
  unbind() {
    ...
    this.ghost.hidden = true;
    if (this.source) setReadOnly(this.source.view, false);
  ...
  async sendMark(record) {
    ...
    if (!at) return;
    const piece = this.source.view.state.sliceDoc(at.from, at.to);

    // KR-17: the piece lands raw, in the source's own Markdown — no quote block
    // added, nothing asked. It lands as its own block: a blank line before it
    // unless it is at the very start, and one after unless the document ends
    // there. Without that it fuses with whatever the cursor was sitting in.
  ...
  showTravelCount() {
    const count = this.marks?.list().length ?? 0;
