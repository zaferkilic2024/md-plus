...
export class MarkStore {
  ...
  onHover = (event) => {
    ...
    // Pinned stays; a preview of ANOTHER mark yields to the badge under the
    // pointer (two commented marks close together, the hand slides across).
    if (!this.strip.dom.hidden) {
      if (this.strip.pinned || this.strip.mark?.id === badge.dataset.mark) return;
    }
  ...
  open(record, { writing = false, anchor = null, preview = false } = {}) {
    const view = this.tab.view;
  ...
  onClick = (event) => {
    ...
    if (!found) {
      this.strip.hide();
      // In the tab only: Aktarma's shade belongs to its bridge (transfer.js).
      if (this.face === "sekme") this.settleTravelShade();
      return;
    }
    ...
    const anchor = badge
      ...
      : this.face === "aktarma"
        ...
        : null;
    this.open(found.record, anchor ? { anchor } : {});
  };
