...
export function createChrome({ onCommand, activeTab, onSearch, onBack }) {
  ...
  const marksTool = tool("marks", t("menu.marks"), () => {
    ...
    const placed = tab.marks.list();
    const menu = popover(
      marksTool,
      placed.map(({ from, to, record }) => ({
        icon: record.yorum ? "note" : undefined,
        label: snippetOf(tab.view.state.sliceDoc(from, to)),
  ...
  const updateMarksTool = () => {
    marksTool.disabled = !activeTab()?.marks?.list().length;
  };
  ...
  const updateContentsTool = () => {
    const tab = activeTab();
    contents.disabled = !tab || outlineOf(tab.view.state.doc.toString()).length === 0;
  };
